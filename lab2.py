...
def zad3(df):
    ...
    processed_data = pca.fit_transform(df)
    processed_df = pd.DataFrame(data=processed_data, columns=[f'COMP{i}' for i in range(processed_data.shape[1])])
    processed_df.to_csv(sep=';', path_or_buf='dane/breast_pca_n.csv')
    print(f'Wariancja po zmniejszeniu wymiarowości: \n{processed_df.var()}\n')
    print('Wariancja wyjaśniona dla poszczególnych składowych:')
    for i, value in enumerate(pca.explained_variance_ratio_):
        print(f'{processed_df.columns[i]}: {value:.5f}')
...
def zad4(df):
    ...
    processed_data = svd.fit_transform(df)
    processed_df = pd.DataFrame(data=processed_data, columns=[f'COMP{i}' for i in range(processed_data.shape[1])])
    processed_df.to_csv(sep=';', path_or_buf='dane/breast_algorithm.csv')
    print(f'Wariancja po zmniejszeniu wymiarowości: \n{processed_df.var()}\n')
    print('Wariancja wyjaśniona dla poszczególnych składowych:')
